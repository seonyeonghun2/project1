window.addEventListener("DOMContentLoaded", () => {
  const loadOneBtn = document.getElementById("getOneUser");
  loadOneBtn.addEventListener("click", async () => {
    loadOneBtn.disabled = true;
    const phoneRegex = /^(\d{2,3})?(\d{3,4})(\d{4})$/;
    const result = await fetch("http://localhost:3000/user/1")
      .then((res) => res.json())
      .then((items) => {
